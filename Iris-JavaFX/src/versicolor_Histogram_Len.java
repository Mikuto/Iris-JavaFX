 ...
 */
public class versicolor_Histogram_Len extends Application {
//	double[] versicolorLen = new double[] {1.4,1.4,1.3,1.5,1.4,1.7,1.4,1.5,1.4,1.5,1.5,1.6,1.4,1.1,1.2,1.5,1.3,1.4,1.7,1.5,1.7,1.5,1,1.7,1.9,1.6,1.6,1.5,1.4,1.6,1.6,1.5,1.5,1.4,1.5,1.2,1.3,1.4,1.3,1.5,1.3,1.3,1.3,1.6,1.9,1.4,1.6,1.4,1.5,1.4};
    ...
    public void start(Stage primaryStage) {
    	...
    	for (int i = 0; i < DATA_SIZE; i++) {// ｘ軸とy軸の最大値を測る
    		maxLen = Math.max(maxLen, versicolorLen[i]);
    		minLen = Math.min(minLen, versicolorLen[i]);
    	}
...
//    }
    ...
    private void groupData(){
        ...
        for(int i=0; i<DATA_SIZE; i++){
            if(versicolorLen[i]<=(maxLen/10)){
                group[0]++;
            }else if(versicolorLen[i]<=((maxLen*2)/10)){
                group[1]++;
            }else if(versicolorLen[i]<=((maxLen*3)/10)){
                group[2]++;
            }else if(versicolorLen[i]<=((maxLen*4)/10)){
                group[3]++;
            }else if(versicolorLen[i]<=((maxLen*5)/10)){
                group[4]++;
            }else if(versicolorLen[i]<=((maxLen*6)/10)){
                group[5]++;
            }else if(versicolorLen[i]<=((maxLen*7)/10)){
                group[6]++;
            }else if(versicolorLen[i]<=((maxLen*8)/10)){
                group[7]++;
            }else if(versicolorLen[i]<=((maxLen*9)/10)){
                group[8]++;
            }else if(versicolorLen[i]<=(maxLen)){
                group[9]++;
            }
        }
    }

 ...
 * @author bitwalk
 */
public class versicolor_S extends Application {
    ...
    public void start(Stage stage) {
		...
		int n = versicolorLen.length; // 150個

		double maxLen = 0;
		double maxWidth = 0;
		for (int i = 0; i < n; i++) {// ｘ軸とy軸の最大値を測る
			maxLen = Math.max(maxLen, versicolorLen[i]);
			maxWidth = Math.max(maxWidth, versicolorWidth[i]);
		}
        ...
        lineChart.setTitle("versicolor");
 
        XYChart.Series series1 = new XYChart.Series();
        series1.setName("versicolorデータ");
		for (int i = 0; i < n; i++) {
			series1.getData().add(new XYChart.Data(versicolorWidth[i], versicolorLen[i]));
		}
		double a = 0, b = 0;
		...
		for (int i = 0; i < n; i++) {
			sum_xy += versicolorWidth[i] * versicolorLen[i];
			sum_x += versicolorWidth[i];
			sum_y += versicolorLen[i];
			sum_x2 += versicolorWidth[i] * versicolorWidth[i];
		}
		...
		for (int i = 0; i < n; i++) {
			bunX += (versicolorWidth[i] - aveX) * (versicolorWidth[i] - aveX);
			bunY += (versicolorLen[i] - aveY) * (versicolorLen[i] - aveY);
			bunXY += (versicolorWidth[i] - aveX) * (versicolorLen[i] - aveY);
		}

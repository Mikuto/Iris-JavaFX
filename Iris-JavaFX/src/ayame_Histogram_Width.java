 ...
 */
public class ayame_Histogram_Width extends Application {
    ...
    public void start(Stage primaryStage) {
    	...
    	for (int i = 0; i < DATA_SIZE; i++) {// ｘ軸とy軸の最大値を測る
    		maxLen = Math.max(maxLen, setosaWidth[i]);
    		minLen = Math.min(minLen, setosaWidth[i]);
    	}
...
//    }
    ...
    private void groupData(){
        ...
        for(int i=0; i<DATA_SIZE; i++){
            if(setosaWidth[i]<=(maxLen/10)){
                group[0]++;
            }else if(setosaWidth[i]<=((maxLen*2)/10)){
                group[1]++;
            }else if(setosaWidth[i]<=((maxLen*3)/10)){
                group[2]++;
            }else if(setosaWidth[i]<=((maxLen*4)/10)){
                group[3]++;
            }else if(setosaWidth[i]<=((maxLen*5)/10)){
                group[4]++;
            }else if(setosaWidth[i]<=((maxLen*6)/10)){
                group[5]++;
            }else if(setosaWidth[i]<=((maxLen*7)/10)){
                group[6]++;
            }else if(setosaWidth[i]<=((maxLen*8)/10)){
                group[7]++;
            }else if(setosaWidth[i]<=((maxLen*9)/10)){
                group[8]++;
            }else if(setosaWidth[i]<=(maxLen)){
                group[9]++;
            }
        }
    }

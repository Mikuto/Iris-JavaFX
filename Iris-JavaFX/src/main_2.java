...
public class main_2 extends Application {
	...
	public void start(Stage stage) {
		...
		double maxLen = 0;
		double maxWidth = 0;
		for (int i = 50; i < 100; i++) {// ｘ軸とy軸の最大値を測る
			maxLen = Math.max(maxLen, PetalLen[i]);
			maxWidth = Math.max(maxWidth, PetalWidth[i]);
		}
		// ｘ，ｙの最大値から0.1足した範囲のグラフ
		maxWidth += 0.1;
		final NumberAxis xAxis = new NumberAxis(0, maxWidth + 0.1, 1);
		final NumberAxis yAxis = new NumberAxis(0, maxLen + 0.3, 1);
		...
		Species1.setName("Iris versicolor");
